(function (global, factory) {
  ...
  (global = global || self, global.Spine = factory(global.karas));
  ...
  function loadSkeleton(assetManager, initialAnimation, skin, atlasUrl, jsonUrl, texUrl) {
    ...
    var atlas = new spineCanvas.TextureAtlas(assetManager.get(atlasUrl), function (path) {
      return assetManager.get(path);
    }); // Create a AtlasAttachmentLoader, which is specific to the WebGL backend.

    var atlasLoader = new spineCanvas.AtlasAttachmentLoader(atlas); // Create a SkeletonJson instance for parsing the .json file.
    ...
    var animationState = new spineCanvas.AnimationState(new spineCanvas.AnimationStateData(skeleton.data));
    animationState.setAnimation(0, initialAnimation, true);
    animationState.addListener({
      event: function event(trackIndex, _event) {// console.log('Event on track ' + trackIndex + ': ' + JSON.stringify(event));
      },
      complete: function complete(trackIndex, loopCount) {// console.log('Animation on track ' + trackIndex + ' completed, loop count: ' + loopCount);
      },
      start: function start(trackIndex) {// console.log('Animation on track ' + trackIndex + ' started');
      },
      end: function end(trackIndex) {// console.log('Animation on track ' + trackIndex + ' ended');
      }
